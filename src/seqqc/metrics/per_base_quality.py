...
class PerBaseQualityCalculator(MetricCalculator):
    ...
    def _ensure_capacity(self, length: int) -> None:
        ...
        while len(self._histograms) < length:
            self._histograms.append(np.zeros(self._MAX_PHRED + 1, dtype=int))
    ...
    def _quantile_from_histogram(hist: np.ndarray, q: float) -> float:
        ...
        low_bin = min(
            int(np.searchsorted(cumsum, floor_h, side="right")), len(hist) - 1
        )
        high_bin = min(
            int(np.searchsorted(cumsum, ceil_h, side="right")), len(hist) - 1
        )
